...
Three further full-screen overlays sit ABOVE all of the above, structurally separate from the tab/sheet system entirely — they're not bottom sheets, and one of them (the login screen) can block the whole app outright:
- **Login / sign-up screen** (section 19.2) — covers everything until the user is authenticated.
- **Forgot password screen** (section 19.3) — opened from the login screen's "Forgot password?" link; has its own request/confirm internal states.
...
### 19.3 Forgot password → dedicated request screen → in-app "set new password" screen
...
**"Forgot password?"** (item 7 in section 19.2) opens a separate full-screen overlay (`#forgotPasswordScreen`, same `.auth-screen`/`.auth-card` visual treatment as login), hiding the login screen underneath. This overlay has two internal states, never both visible at once:

- **Request view (shown first):** an Email field (pre-filled from whatever was already typed on the login screen, if anything), an error area, a **"Send reset email"** button (primary — same disable + spinner-and-"Sending…" loading treatment as the login submit button, section 19.2, item 6), and a **"Log in"** button that discards everything and returns to the login screen with no request sent.
- **Confirm view (shown after a send attempt that isn't a hard error):** a message and a single **"Log in"** button that returns to the login screen.

Tapping "Send reset email" with an empty field shows an inline error and stays put. Otherwise, call `auth.sendPasswordResetEmail(email)` and branch on the outcome:
- **Success, OR a failure whose error code is `auth/user-not-found`:** switch to the confirm view with the exact same message either way: **"If [email] is registered with us, an email will be sent to that address with a link to reset your password."** This wording is deliberate and must never differ based on whether the account actually exists — surfacing that distinction (e.g. a literal "no account with that email" error) would let anyone use this form to probe which email addresses have accounts here, which is a real information leak this app specifically avoids.
- **Any other failure** — surface it as an actual inline error and stay on the request view so the user can fix it and retry: `auth/invalid-email` → "That doesn't look like a valid email address."; `auth/too-many-requests` → "Too many attempts — please wait a bit and try again."; `auth/network-request-failed` → "Network error — check your connection and try again."; anything else → Firebase's own error message text as a fallback.

Separately, and regardless of how the user got there: on page load, check `window.location.search` for `mode=resetPassword` and `oobCode` (both present in the link Firebase's email sends, once the action URL above points back at this app). If found: call `auth.verifyPasswordResetCode(oobCode)` — this both validates the code and resolves the account's email address. On success, hide the normal login screen and show a THIRD full-screen overlay (`#resetPasswordScreen`, same `.auth-screen`/`.auth-card` visual treatment) reading **"Set a new password for [email]"**, with a single new-password field (same show/hide toggle pattern as section 19.2) and a **"Set new password"** button. Client-side, require at least 6 characters before allowing submit (inline error otherwise). On submit, call `auth.confirmPasswordReset(oobCode, newPassword)`; on success, close this screen, strip the query string from the URL (`history.replaceState`, so refreshing doesn't re-trigger the flow), re-show the normal login screen, and show a single-button confirm notice: **"Password updated — you can now log in with your new password."** On failure (expired/invalid code) at either the verify or confirm step, show a single-button notice with Firebase's error text and clean up the URL the same way.
...
5. **"Change password"** — hidden entirely if the account's only sign-in provider isn't `"password"` (i.e., there's nothing to change for such an account.) When shown: opens a yes/no confirm ("Send a password reset link to [email]?"); on confirm, `auth.sendPasswordResetEmail(currentUser.email)` directly (this entry point skips the login screen's request/confirm screen, section 19.3, since the email is already known and confirmed here — it goes straight to sending, then a simple single-button notice). Whichever way the reset email was triggered, clicking the link inside it always lands on the SAME "set new password" screen (section 19.3) — that page doesn't know or care whether it was reached via My Account or via the login screen's "Forgot password?".
6. **"Close"**.
...
- [ ] The login screen blocks the entire app until the user signs in or signs up with email/password; the submit button shows a spinner and disables itself while the request is in flight, and re-enables with an error message on failure. "Forgot password?" opens its own request screen (email field + Send button + a Log in button to bail out); after sending, it shows the SAME generic confirmation message regardless of whether that email actually has an account, plus a Log in button back to the login screen — a genuine send failure (bad format, rate-limited, offline) shows a real error instead and stays on the request screen. Following the emailed link opens an in-app "set new password" screen (not Firebase's generic hosted page) that successfully updates the password and returns to login.
